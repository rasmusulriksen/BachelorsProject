...
public interface IMessageQueueRepo
{
    ...
    Task<long> EnqueueMessage(JObject message, string eventName);
    ...
    Task<IEnumerable<IdAndJObject>> DequeueMessages(string referer, int count);
    ...
    Task MarkMessageAsDone(long messageId, string referer);
}
